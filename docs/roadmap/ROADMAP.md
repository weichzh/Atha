...
在现有受控书根和同一阅读内核上交付 ZIP 图片序列：先研究 Readest / foliate-js 的 CBZ 约束和成熟 ZIP / 图像方案，再用原创小型图片包锁定自然排序、基础元数据、封面、分页、损坏图片与 ZIP 安全边界。Android 正式入口必须覆盖系统 picker、首屏、翻页、强停恢复和内存证据；不为后续格式预建通用工厂。

停止条件：原创 CBZ fixture 的解析、安全和 Windows / Android 主循环同时通过，或证据表明应复用一个成熟库而不是扩展现有 ZIP seam。

## Next：按格式风险逐片交付

1. **Markdown / TXT**：转换为现有 Book / section 模型，不建立第二套阅读器；
2. **FB2**：优先采用成熟解析库并保持相同 Locator 与安全渲染边界；
3. **MOBI / AZW / KF8 / AZW3**：借鉴 Readest 的 foliate-js 能力边界，最终方案先过许可证审查，并以 Android 真机内存、冷开、翻页和重排门槛验收；
4. **CSS 编辑器与模块管理**：每书覆盖、可组合模块、预览、撤销、导入导出和失败回退；
5. **CSS 社区**：GitHub 仓库作为存储与 review 边界，用户登录 GitHub 后以 pull request 投稿，不自建账号或审核后端；
6. **阅读统计**：本地优先记录阅读时长、进度与连续阅读，先明确暂停、后台和跨设备语义；
7. **离线词典**：先实现本地索引、查词与安全释义渲染，再用指定 MOBI / MDict 样本在 Android ARM 真机做基准；AGPL 兼容不替代词典内容版权与分发审查。
