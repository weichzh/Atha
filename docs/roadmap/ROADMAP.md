...
离线词典关闭后，以 PCT-AL10 上已安装的微信读书与 Readest 原图、真实拖动和滑动行为为准，重做右上设置菜单、首行缩进、字体滑块和翻页方式。只保留左右分页滑动与上下滚动两种阅读模式；拖动必须连续反馈，字号档位覆盖微信读书同级范围，并明确区分 CSS 字号、系统 DPI 与设备像素换算。设置入口参考 Readest 的层级、动效和关闭行为，不引入通用动效框架。
