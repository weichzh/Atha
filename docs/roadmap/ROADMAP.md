...
指定 MOBI / MDict 本地样本、当前 Kindle 词典早拒绝和成熟 parser / 索引方案已复核并落地。切片只实现本地索引、精确查词和安全释义渲染；释义现保留固定白名单内的语义结构，并由应用统一提供词典排版，不加载来源 CSS、富 MDD 资源或网络内容。既有 Linux 与 PCT-AL10 性能、应用 PSS、真实长按和直接查词均已通过；富文本变更当前只完成 Linux 后端与合成浏览器视口检查，真机样式仍需重新验收。
