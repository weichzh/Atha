...
- 资料库“阅读记忆”复用 MessageStore 既有 FTS5、Edition、当前根 Anchor 和同一 Snapshot 渲染函数做跨书搜索；三字符以上按 FTS 相关性、更新时间与消息身份稳定排序，短查询使用 escaped `LIKE` 并按更新时间与消息身份排序，均最多返回 200 条。已删除命中或根 Message 已墓碑的 Conversation 不进入结果；查询和结果内容不写产品日志。
- 搜索结果只在书架存在相同完整内容身份时显示“跳回原书”。资料库只传递受限消息身份，阅读器打开对应 Edition 后重新加载 Conversation，核对根 Message，再通过当前 Anchor 定位或唯一重锚；位置失效时只报告 Snapshot 降级，不把历史捕获伪装为当前原文。
