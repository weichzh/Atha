...
资料库主窗口提供“书架 / 阅读记忆”两个同层入口。最近阅读只从经过 schema 1 完整校验的应用级统计记录匹配当前书架完整内容身份，并按最后本地阅读日降序、内容身份稳定排序；缺少统计表示尚无记录，损坏或不可访问则明确禁用投影。跨书消息搜索与 Snapshot 由 MessageStore 负责，书架层不复制 Message 或 Locator 事实。
