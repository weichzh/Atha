...
默认样式提供稳定、克制的阅读体验。Preferences 把主题、亮度、字号、字体和三档阅读密度作为应用默认值，把书源样式开关与用户 CSS 作为本书覆盖；R5 起两层分别校验、恢复和持久化。阅读密度只组合绝对像素页边距与行高，页面设备像素契约不变；亮度只过滤固定阅读页，不改变壳层控件。
...
正式内容回归在 `devicePixelRatio = 1` 时使用 780 × 1680 竖屏视口；4K、200% DPI 的界面截图使用 390 × 840 CSS 像素和 DPR 2。阅读器的一页是有独立上下边距的分页内容区，不是任意滚动位置的截图：不得裁切文字行、公式或图形。首个临时字号基线为 32px、行距为 1.6；它只是首个可验收切片的参数，后续仍可调整。
