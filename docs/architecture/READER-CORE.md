...
默认样式提供稳定、克制的阅读体验。`reader/theme.css` 是应用壳与阅读壳颜色 token 的唯一事实所有者，并随阅读样式进入书籍 Shadow DOM 与 Snapshot；功能 CSS 只声明布局和语义引用。Preferences 把系统/浅色/纸张/深色书页主题、亮度、16–40 逻辑 CSS px 字号、字体和三档行距作为全书阅读默认值，把左右翻页 / 上下滚动、书源样式开关、三档左右边距、段首缩进、段距和有序 CSS 模块作为本书覆盖。应用壳另用 `atha.app.appearance.v1` 保存跟随系统 / 浅色 / 深色界面主题；它只影响书架、阅读记忆和应用设置，不复用或改写阅读主题。旧的单段 `userStylesheet` 恢复时迁移为 `legacy-user-css` 模块；旧阅读默认记录中的四个自由边距字段和点击 / 滑动开关继续忽略。上下正文安全区固定为 144 设备像素，Android 只在顶部 / 底部安全区更大时扩大对应排版边距；左右边距默认 32，可按书选择 24 / 32 / 48 设备像素。亮度只过滤阅读页，不改变壳层控件。
...
Tauri 无启动书籍参数时显示 Svelte 书架，并通过官方文件对话框选择一个或多个 EPUB、CBZ、FB2 / FBZ、MOBI / AZW / AZW3、Markdown 或 TXT。桌面资料库还接收 Tauri 原生文件拖放；Linux / Windows 文件关联只在冷启动参数全部是受支持书籍路径时导入全部有效文件并打开第一本，普通启动仍进入资料库。系统选择器、拖放与关联启动共用同一 staging 和 `ImportReport`，路径 command 只允许资料库根路由、每次至多 32 个非空有界路径；Android 不注册桌面关联，继续使用 SAF。书架网格 / 列表只是同一 `LibraryBook[]` 与书卡 DOM 的非持久布局投影，搜索、排序、进度、选择和操作不分叉。

`reader::library::LocalLibrary` 是书架边界，只暴露列出、登记、导入、打开、读取 / 更换 / 恢复封面、移出书架和删除本地书籍数据；已知 `.epub`、`.cbz`、`.fb2` / `.fbz`、`.mobi` / `.azw` / `.azw3`、`.md` / `.markdown`、`.txt` 严格分派到对应 importer。Android content URI 由 Tauri 内置 PathPlugin 经 `ContentResolver` 取得显示文件名，只保留允许列表后缀后复制到 Picker cache；provider 不返回可用后缀时稳定拒绝，不从 URI 或正文猜格式。加入书架先把源文件耐久写入 `SourceBooks`、计算既有内容身份并写受限记录，再在同一 blocking worker 中调用既有打开流程发布 `ImportedBooks`，因此返回书架时已有 importer 元数据与内置封面；以后直接打开缓存。同进程准备由共享锁串行发布，缓存命中先核对格式 marker 与元数据，再核对 manifest 及其声明的全部 section / resource；EPUB v2 至 v5 完整缓存都可读，有耐久源时才尝试把 v2 至 v4 升级到 v5，失败仍回退旧缓存。缺失、空 section 或损坏产物由任一同身份耐久后缀重建。再次登记同一内容可原子修复身份异常的耐久源或重建书架记录。EPUB / CBZ 保持原始文件 SHA-256，Markdown / TXT 使用各自格式域，FB2 / FBZ 使用解包后 FB2 XML 与固定格式域生成身份，Kindle 三种后缀使用同一固定格式域，因此相同字节不会因封装后缀产生副本。移出书架删除书架记录和自定义封面；删除本地数据还删除同身份耐久源、导入缓存和本书浏览器状态，但始终保留 Message、Anchor、Snapshot 与资产。

EPUB importer 从 OPF 有界提取标题、至多 16 位作者和一个受支持的封面资源；EPUB2 的 `meta name="cover"` 与 EPUB3 的 `cover-image` 最终投影为同一封面字段。CBZ 只消费可选 `ComicInfo.xml` 中有界的 `Title`、`Writer` 与唯一有效 `FrontCover`。FB2 从 `description/title-info` 有界投影书名、作者和封面引用。无效、冲突或超限的可选元数据不会扩大受信任内容边界；无封面时由壳层显示占位。自定义封面只接受 16 MiB 内、单边不超过 8192 且总像素不超过 20000000 的 JPEG、PNG 或 WebP，使用完整书籍身份命名并优先于内置封面；崩溃恢复会清理 staging 或恢复上一个完整版本。Svelte 只接收书籍身份、标题、作者、封面可用性、自定义封面状态、导入时间和是否已准备，不接收源路径、缓存路径或书籍内容。宿主把动态 `atha-book` 根切换到已校验缓存；`atha-cover` 以 `no-store` 返回当前内置或自定义封面。书架沿用 Readest 的选择文件、内容哈希去重、耐久目录和打开链路，不采用其同步、分组、转换队列、多来源或全局状态结构。

书架搜索与排序只投影内存中的受限 `LibraryBook[]`：搜索匹配标题 / 作者，默认保持后端导入顺序，另提供稳定的书名与作者顺序。进度页只读同源 schema 1 进度记录，并复用阅读器的内容版本、Locator、大小和精确字段约束；合法同书记录只表示“在读”，缺失或无效记录表示“未开始”，存储不可访问时禁用进度投影而不伪造状态。触控长按进入既有选择模式并显示底部操作栏；桌面右键及键盘上下文键显示同一单书操作集合。批量操作串行移出或删除本地数据，封面更换只在单书上下文可用；普通模式没有常驻破坏性按钮。封面使用浏览器原生 lazy loading / async decoding，移动端保持三列，未引入虚拟列表、同步状态或虚拟化。

资料库主窗口提供“书架 / 阅读记忆 / 设置”三个同层入口。设置页只承载应用外观、全书阅读默认恢复、离线词典管理和本地资料维护；阅读排版与本书覆盖仍留在阅读界面。恢复阅读默认只重置书页主题、亮度、字号、字体和行距，不改变应用主题或任一本书的覆盖。最近阅读只从经过 schema 1 完整校验的应用级统计记录匹配当前书架完整内容身份，并按最后本地阅读日降序、内容身份稳定排序；缺少统计表示尚无记录，损坏或不可访问则明确禁用投影。跨书消息搜索与 Snapshot 由 MessageStore 负责，书架层不复制 Message 或 Locator 事实。

`backend::local_data::LocalData` 是应用级本地数据协调边界。schema 1 `.atha-data` 只包含规范化书架记录、对应自定义封面、全部 `SourceBooks`、离线词典、嵌套 MessageStore `.atha-backup` 和生产 localStorage allowlist；`ImportedBooks`、Picker cache 与日志不进入制品。恢复在当前数据根内完成唯一 entry、路径、数量、容量、哈希、书源身份、自定义封面、词典、MessageStore 和浏览器状态校验，再以同文件系统目录切换和 SQLite Online Backup 发布。`prepared` 表示尚未发布，第一次目录切换前耐久写入 `publishing` marker；启动看到未提交发布时回滚，`committed` 等待 WebView 同步替换浏览器状态并确认；文件数上限 100,000，总展开上限 8 GiB，浏览器状态上限 16 MiB。

应用设置页提供完整备份、完整恢复和书籍文件、阅读缓存、消息与快照、词典、阅读设置的分类占用；搜索栏不再承载管理菜单。批量选择同时提供“移出书架”和“删除本地数据”，使用不同确认文案；恢复的浏览器状态替换失败会回写操作前 allowlist，删除则保留耐久 intent 与已经完成的定点清理，在重载后幂等续做，两者都不读取或改写其他 origin 数据。缺少耐久原文件的旧书架项不会伪装成完整备份，界面要求用户重新选择原文件补齐后再试。Tauri `local_data_maintenance` 只接受主窗口资料库根路由，SAF 继续复用 `platform_file`，产品日志只记录操作、固定阶段、稳定错误码和耗时。
...
从阅读记忆跳回时，资料库先按完整内容身份打开当前书籍，只携带 32 字节十六进制 Conversation、命中 Message 与根 Message 身份。阅读页加载当前 MessageStore Conversation 并核对 Edition 与未删除根 Message，再使用 Annotation 的同一 Locator / 唯一重锚路径定位；成功后打开命中对话，失败则明确标记为仅可查看历史 Snapshot。
...
笔记入口负责新建、为 source-only Message 添加正文和预填编辑，并打开定位根消息的半屏对话。标注笔记 dialog 与消息输入器共用 `VisualViewport` 几何；软键盘缩小可视区域时，当前输入和保存 / 发送操作保持可见。消息输入器使用最长 8000 字符的受限 Tiptap JSON，由后端派生纯文本；可视编辑与原始 Markdown 共用同一耐久事实，Markdown 无法表示的结构会保留原内容并拒绝切换。全屏笔记页投影所有未删除根 Message，并提供章节 / 全文筛选、本书导出和对话入口；对话浮层负责回复、引用、修订、关系、快照和跳回。删除写入 Message 墓碑并立即撤销正文投影。标注颜色、notebook、同步和 tombstone 压缩留待后续真实需求。
...
预览只设置独立图片元素的已校验本地 URL、安全标题和替代文本，不复制书源 HTML、样式或脚本。普通图片始终保留原色，公式预览沿用正文的明暗主题过滤；查看层提供 50%–400% 缩放、复位与放大后的原生双向滚动，双指以中心点连续缩放，桌面滚轮以指针位置连续缩放。关闭后焦点返回原图片，且不改变 section、页码或 Locator。图集、保存和 OCR 等能力在出现明确需求前不建设。
...
表格预览深克隆已经过书籍安全清洗的表格 DOM，以保留 caption、行列、跨度、公式和图片；投影再次移除链接目标、事件属性、行内样式、焦点属性与非公式 class。查看层先打开，尚未加载的公式保持同尺寸空白，并复用正文 SVG 安全校验以最多三个并发按表格顺序渐进加载；worker 等待真实 `load`、`error` 或 `abort` 终态后复用，关闭则立即取消剩余队列并阻止在途结果回写。表格与图片 / 公式共用同一双指、滚轮和按钮缩放状态；代码预览只设置 `textContent` 并保留空白。两者关闭后恢复焦点且不改变 section、页码或 Locator；复制按钮、导出、执行和编辑留待明确需求。
...
`scripts/check-reader-linux.sh` 从 Rust 测试 writer 生成原创 FB2，并在仓库 `.tmp` 下种入隔离的真实 `LocalLibrary`。入口运行 Svelte 与 Tauri build，再用官方 `tauri-driver` / WebKitWebDriver 驱动当前 Linux Tauri 壳；测试专用启动环境变量只接受 `360x760`、`600x760`、`1000x760`、`1280x800` 与 `1600x900` 五个固定尺寸，避免把 WebDriver 不生效的运行时 resize 当成多视口证据。门禁覆盖拖放事件、网格 / 列表、资料库管理、阅读记忆、桌面工作区、完整导入诊断、跨 section 回退、当前手势矩阵和 AppLog 隐私。

当前 WebKitGTK 2.52.5 的 W3C touch Actions 会挂起，因此 Linux 门明确请求可信 `mouse` Pointer Actions，逐次核对真实命中、`isTrusted`、左右点按 / 横拖、媒体误开、纵向意图、宽表双向内部滚动与双向边界移交，并记录请求与实际 `pointerType`。该门只算可信自动化指针与主线程性能证据，不冒充实体触摸；PCT-AL10 自动滑动、SurfaceFlinger presentation 与用户自然手指触摸分别保留真实目标证据边界。
...
公式密集章节只对具有合法显式宽高的 SVG 公式启用延迟资源：合法 `width` / `height` 会显式投影为 `aspect-ratio`，使无 `src` 的旧 WebView 也按加载后的内在比例形成同尺寸占位；首个稳定页和每次翻页优先当前页与下一页，横向收束期间向左额外覆盖一页，避免回划目标页落在加载范围外，其余公式进入相邻视口时再加载。稳定显式尺寸图片的可见性扫描缓存书内坐标，横向翻页不重复读取每张图片的 viewport geometry；布局或样式变化时整体失效。校验通过前不设置可呈现 `src`；SVG 获取、解析和安全校验完成后，设置 `src` 的终态等待窗口为 50ms，超时则公式保持同尺寸空白并由迟到 `load` / `error` 局部收尾。该 50ms 不约束前置 SVG 校验的 wall time。可见资源写入前捕获一次 Locator，并比较书页、资源及其父元素的前后几何；成功加载且几何不变时不重排，成功加载后几何变化或失败占位替换则走既有重排并恢复 Locator。迟到成功也只在几何变化时进入现有合并重排，迟到失败总是进入该入口；离开 live 章节即释放解码和几何状态，最多三个已校验 detached section 只保留重建 DOM 所需数据。基准已经否定普通正文图片、整页或整节等待全部资源的揭示闸门，因此空白占位只稳定单个公式资源盒。
