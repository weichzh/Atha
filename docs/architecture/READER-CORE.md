...
默认样式提供稳定、克制的阅读体验。Preferences 把主题、亮度、字号、字体、三档行距和四边页边距作为应用默认值，把书源样式开关与用户 CSS 作为本书覆盖；R5 起两层分别校验、恢复和持久化。行距与页边距独立，四边距使用固定页面设备像素并可分别调整，系统缩放不改变排版；亮度只过滤固定阅读页，不改变壳层控件。
...
`Navigation` 组合 reading session、Locator 与 pagination，统一处理页内移动、section 边界和 TOC 跳转。移动阅读壳层默认沉浸，点击正文中央临时显示覆盖层；目录使用一个原生 TOC 列表，书签作为对应章节下的目录项，添加与取消只由右上角书签入口触发。
...
阅读页使用固定 780 × 1680 设备像素画布。页内字号、边距、栏宽、公式和图形尺寸都使用该画布的绝对像素坐标；显示层只以 `1 / devicePixelRatio` 抵消系统 DPI，不再根据窗口大小缩放页面。Windows 窗口、覆盖工具层和错误提示仍使用系统逻辑像素并遵循 DPI；窗口不足时由壳层滚动承载固定页面。

正式内容回归在 `devicePixelRatio = 1` 时使用 780 × 1680 竖屏视口；4K、200% DPI 的界面截图使用 390 × 840 CSS 像素和 DPR 2。阅读器的一页是有独立四边距的分页内容区，不是任意滚动位置的截图：不得裁切文字行、公式或图形。移动横排默认值参考 Readest 的左右 16、带页眉页脚上下 44 CSS 像素，并换算为固定画布中的左右 32、上下 88 设备像素；首个字号基线为 32px、行距为 1.6，均可从阅读设置调整。
