...
| `reader/web/` | Locator、导航、偏好、翻页输入、阅读会话、内容安全与加载、分页与公式布局、诊断与 benchmark、页面组合入口 | M2 R4A 已验证 |
...
- 九份页面源码由应用资源协议按固定顺序交付为单个 `atha-reader.mjs`，避免为源码分层增加多次自定义协议请求；浏览器验证服务器使用同一顺序；
