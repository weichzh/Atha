...
- Tauri `lib.rs` 组合状态、窗口、protocol、lifecycle 与 command 注册，并暂时保留 library、telemetry 与 protocol adapter；书架 command 只向可信壳暴露受限书目，不返回源路径或内容；`message_commands` adapter 统一校验当前阅读窗口并转发受限 DTO，`message_maintenance` adapter 只接受资料库根路由并在 blocking worker 执行全库备份 / 恢复；动态 `atha-book` 提供当前正文，独立 `atha-cover` 只读提供已登记封面；阅读器遥测仍复用后端解析和共享 diagnostics；消息专项检查精确核对 handler 注册与 permission；
...
- 当前选中文字入口在真实鼠标 Range 附近提供复制、标注和笔记；原生 copy 事件不写记录。Tauri 产品把 highlight 与 note 投影到同一根 Message，底栏笔记页可筛选和导出，列表项进入对话浮层；浮层提供本条、本章和本书三种范围，聚合范围可切换时间或书序，进入单条后提供回复、引用、编辑、删除、修订、关系、历史快照和跳回，标题栏只保留返回与全屏；
...
- 半屏/全屏富文本输入完成后，消息专项、四样本、书架和 Tauri 产品回归通过；修复全屏宽屏定位与 Markdown 无损门禁后的基准 `1785936897004-15436` 的 P95 为冷启动 632.048ms、首稳 117.500ms、热开 20.800ms、翻页 6.900ms、字号重排 55.600ms，均低于既有门槛。首次打开对话时才加载普通可视编辑器，切换输入模式时才加载 Markdown 转换；主 reader bundle 未引入编辑器依赖；
- 标记范围查询完成后，后端使用一次 Tauri IPC 返回当前 Edition 或 section 的完整 Conversation；时间和书序只在前端投影，不复制消息事实。消息专项与 Tauri 产品回归通过；基准 `1785938288886-20832` 的 P95 为冷启动 597.112ms、首稳 133.700ms、热开 20.900ms、翻页 6.800ms、字号重排 41.900ms，均低于既有门槛；
- 直接 `--epub` 启动把 importer 已解析的内容版本、书名和作者交给消息运行时，原生导出不再为新会话产生“未命名书籍”；Tauri 产品检查固定断言指定样书书名。真实消息闭环、重启、同内容新路径重导和原生 ZIP 导出已在隔离应用数据中通过；基准 `1785941967575-37292` 的 P95 为冷启动 613.274ms、首稳 129.400ms、热开 22.300ms、翻页 6.800ms、字号重排 48.400ms，均低于既有门槛；
- 单对话导出递归补齐跨对话引用目标的回复父链；公开归档检查使用严格 manifest 类型，并重新执行正文 schema、Locator、快照、资源、关系与资产哈希校验。写出和读取共享 1026 个 entry、64MiB manifest 与 512MiB 总展开大小门槛，写出完成后自检，失败不保留半成品；
...
- P0 schema 与 10,000 条 SQLite CLI benchmark 仍只是历史对照；正式数据库已有版本化迁移和完整 MessageStore 备份 / 恢复，但尚未建立加密、checkpoint、全应用备份或跨设备同步；
