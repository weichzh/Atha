...
图片、表格和公式预览缺少触控与桌面连续缩放；书架的选择和书籍操作依赖显式按钮，首次导入又可能在准备完成前显示无封面。阅读设置入口位于顶栏但面板从底部展开，应用壳与阅读页的主题颜色分散在多个 CSS 文件，手机软键盘还会遮住消息输入器。这些问题共同破坏了输入方式、入口位置和视觉状态的一致性。
...
- 消息输入器使用 `VisualViewport` 跟随可视区域，在软键盘出现时保持编辑器和操作按钮可见；
...
- `INTERACTION-KEYBOARD-01`：可视视口缩小时，普通和全屏消息输入器均位于软键盘上方，列表可滚动且当前输入保持可见；无 `VisualViewport` 时保持既有布局。
...
5. 用 `VisualViewport` 的 resize / scroll 更新消息层可视高度和偏移。
...
阅读设置复用原面板并统一移到底部第五个入口。`reader/theme.css` 成为唯一颜色调色板，同时进入文档、书籍 Shadow DOM 和 Snapshot；功能 CSS 的颜色字面量检查作为回归门。消息层通过 `VisualViewport` 的高度与顶部偏移约束普通、全屏和展开输入器，软键盘出现时当前输入保持在可视区域。
...
- 静态 / 本地：`pnpm --dir reader/app check` 为 0 error / 0 warning，production build 通过，`node --test reader/app/tests/library.test.ts` 为 13 / 13 通过；功能 CSS 没有颜色字面量。
- Linux 真实壳：`bash scripts/check-reader-linux.sh` 在 WebKitGTK 0.55.1 通过。隔离冷导入立即显示封面；360–1600 px 视口无横向溢出；右键菜单和 500 ms 长按底栏状态正确；图片预览滚轮到 127%、合成双指到 160%；阅读设置只在底栏；模拟 320 px 高可视视口时对话层与输入器底部均为 344 px；220 次既有可信鼠标手势和 AppLog 隐私检查继续通过。
