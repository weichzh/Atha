...
首轮最终双轴复核进一步发现单对话导出没有补齐跨对话被引用回复的全部父链，归档检查只验证 ID 关系而没有重新验证正文、Locator、快照和关系语义，写出端也未应用读取端的容量上限。导出闭包现递归补齐回复父项；归档 manifest 改为严格类型并复用正式正文、Locator、Snapshot 与资源校验；写出前后与读取端共享 1026 个 entry、64MiB manifest 和 512MiB 总展开大小门槛，超限时不留下半成品。篡改修订、快照主题、原文哈希或关系类型均被公开检查接口拒绝。
