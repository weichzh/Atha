...
- benchmark 遥测携带并记录当次真实内部页面宽高，不为性能模式保留固定页面例外；
...
- 真实目标证据：当前 Windows 的 Tauri 产品入口可最大化；原生验证按窗口 DPI 和样式把 360 × 640 逻辑客户区换算为所需外框，并确认 `WM_GETMINMAXINFO` 的最小拖动尺寸不低于该值；EPUB 导入 smoke 通过。
- 性能证据：Tauri 基准 `1785772760201-17932` 记录的真实内部页面为 860 × 1640 设备像素，10 样本 P95 为冷启动 713.724ms、首稳 146.000ms、热开 34.700ms、翻页 6.800ms、字号重排 48.600ms，均低于既有门槛。
