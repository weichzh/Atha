...
- 范围修改复用浏览器原生选区，并通过现有 SourceAnchor 与 `replaceAnchor` 保存；
...
- [x] 阅读正文和壳层任意位置的右键操作都不会打开 WebView 默认菜单，也不触发翻页或工具层切换；
- [x] 鼠标或触摸命中已有标注后恢复其正文范围，动作栏可复制、更新范围、添加或编辑笔记和删除；
- [x] 更新范围后原记录身份和笔记保持不变，重排、切章和重载后投影到新范围；
- [x] 编辑笔记时显示已有内容，保存后列表与正文状态立即更新；删除后记录成为 tombstone，正文投影和列表项消失；
- [x] 笔记页填充整个视口，项目正文跳转后关闭工具层，项目编辑和删除不会误触发跳转；
- [x] 新建选区、损坏记录保护、长度限制、唯一原文重锚和持久化能力不回退；
...
- 阅读器根入口统一阻止默认 `contextmenu`；已有标注可从正文恢复选区，执行复制、重选范围、添加或编辑笔记和删除；
- 笔记列表的正文、编辑和删除动作分离，正文跳转后关闭工具层；笔记页与目录共用全屏几何；
- 继续使用既有 SourceAnchor、`replaceAnchor`、tombstone 和纯文本对话框，没有增加依赖、持久化 schema 或第二份标注状态。

## Review

- 规格 review 与工程 review 最终均无 blocking；
- 工程 review 首轮发现新选区可能继承旧 `selectedAnnotationId`，已在共享 `pointerdown` 路径清除并补回归断言；
- 工程 review 指出的目录与笔记全屏样式重复已合并；复核确认没有新增问题或范围外改动。

## Evidence And Residual Risks

- 静态证据：JavaScript 语法、PowerShell 解析、Svelte check、生产构建、Rust 检查与测试、`git diff --check` 均通过；
- 真实目标证据：`scripts/check-reader-samples.ps1` 在真实浏览器完成四样书检查，覆盖真实右键、已有标注命中、重选、笔记编辑和删除、持久化，以及 390×840、960×720、780×1680 三种笔记全屏视口；
- 本地证据：`scripts/check-tauri-reader.ps1` 通过；基准 `1785844847084-31284` 的冷启动、首次稳定、热打开、翻页和字体重排 P95 分别为 736.862、148.500、34.800、6.700 和 41.600 毫秒，均低于门槛；
- 剩余风险：未在物理触摸硬件上直接验证“轻触已有标注”；自动检查覆盖触摸选区，同一 `pointerup` 命中路径没有发现代码分歧。
